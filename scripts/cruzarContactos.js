...
const { leerDevoluciones, escribirTelefonos, asegurarEncabezados, leerConsolidado } = require('../src/googleSheets');

async function main() {
  console.log('Leyendo consolidado de conductores desde Google Sheets...');
  const contactos = await leerConsolidado();

  console.log('Leyendo devoluciones desde Google Sheets...');
  const registros = await leerDevoluciones();
  if (!registros.length) { console.log('Sin filas en el Sheet de devoluciones.'); return; }
  ...
  for (const r of registros) {
    const patente  = String(r.data['PATENTE'] || '').trim().toUpperCase().replace(/-/g, '');
  ...
  console.log(`Encontrados: ${conTel} / ${updates.length} patentes con telefono`);

  console.log('Escribiendo telefonos en columna P del Sheet de devoluciones...');
  await escribirTelefonos(updates);
  console.log(`Listo. ${conTel} telefonos actualizados.`);
}
